...
// создание массива с известными значениями
int[] arraySample1 = [1,2,3,4];
// вывод массива в консоль
foreach (var x in arraySample1)
    Console.WriteLine(x);

// заполнение массива с консоли
Console.WriteLine("Введите цифры, по одной за раз");
for(int i = 0; i < 4;i++)
    int.TryParse(Console.ReadLine(), out arraySample1[i]);
foreach (var x in arraySample1)
    Console.WriteLine(x);

// массив с неизвестным кол-вом ячеек
int.TryParse(Console.ReadLine(), out int length);
int[]  arraySample2 = new int[length];
for(int i = 0; i < length;i++)
    int.TryParse(Console.ReadLine(), out arraySample2[i]);
foreach (var x in arraySample2)
    Console.WriteLine(x);
*/